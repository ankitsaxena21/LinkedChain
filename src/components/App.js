...
class App extends Component {
  ...
  async createQuestion(content) {
    ...
    const linkedChain = web3.eth.Contract(LinkedChain.abi, networkData.address)
    this.setState({ linkedChain })
      await linkedChain.methods.createQuestion(content).send({ from: this.state.account }).then(setTimeout(function(){window.location.reload(true)},9300))
}
...
renderExtras() {
  ...
  {
    header: ( <div><Label size="large">Want a Protection from Deep fakes?</Label></div>),
    description:
      ( <div><hr /> <a href="https://deepfake-protector.herokuapp.com/"><Button primary size="large">Use Deepfake Protector</Button></a>
        </div>),
    meta:(<div><Label as='a' color='green' size="mini" tag>
        upload your files
      </Label></div>)
...
renderAnswers({match}) {
  ...
  return <Answers q={id} answers={answers} tipAmounts={this.state.tipAmounts} authors={this.state.authors} />;
}
