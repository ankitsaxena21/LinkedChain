...
class Answers extends Component {
  ...
  async tipAnswer(q, index, tipAmount) {
    ...
    await linkedChain.methods.tipAnswer(q, index).send({ from: accounts[0], value: tipAmount }).then(setTimeout(function () { window.location.reload(true) }, 28000))
  }
  async createAnswer(q, content) {
    ...
    const networkData = LinkedChain.networks[networkId]
    const linkedChain = web3.eth.Contract(LinkedChain.abi, networkData.address)
    await linkedChain.methods.createAnswer(q, content).send({ from: accounts[0] }).then(setTimeout(function () { window.location.reload(true) }, 26400))
